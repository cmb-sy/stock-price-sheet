...
```javascript
    google.script.run.withSuccessHandler(function (data) {
      state.headers = data.headers; state.rows = data.rows;
      // 売買履歴は新しい取引が上に来るよう日付降順を既定にする（sortDir は selectTab で
      // 既に 'desc'）。ユーザーが他キーを選んだ後の再読込ではその選択を尊重する。
      if (isHistoryTab() && !state.sortKey) state.sortKey = '日付';
      buildSortUI();
      render();
```

- [ ] **Step 2: sortOptions に売買履歴分岐を追加**

`webapp/index.html:709-722` の `sortOptions` を置換:
...
5. ソートが日付降順を既定に、約定単価/株数/銘柄名でも並べ替えできる。
6. 同一銘柄の2取引のうち片方を削除でき、削除ボタンが「この取引を削除」と表示される。
7. AI分析コメントが編集 UI に現れない（読み取り専用）。
8. 既存の保有銘柄/監視/用語解説タブの表示・編集・削除・ソートが従来どおり（リグレッション無し）。
