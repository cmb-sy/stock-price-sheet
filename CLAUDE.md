...
| 取得株価     | manual              | acquisition price (read by Track A for 評価損益; not written) |
| 取得株数     | manual              | shares held (used to compute 配当金 / 評価額 / 評価損益) |
| 配当利回り   | **Track A**         | yfinance `dividendYield` (a percent number, e.g. 2.34) |
| 配当金       | **Track A**         | `dividendRate` × 取得株数 = total annual dividend   |
| 評価額       | **Track A** (derived) | 現在株価 × 取得株数                                |
| 評価損益額   | **Track A** (derived) | (現在株価 − 取得株価) × 取得株数                   |
| 評価損益率（%） | **Track A** (derived) | (現在株価 − 取得株価) / 取得株価 × 100            |
| 目標との乖離率（%） | **Track A** (derived) | (目標売却株価 − 現在株価) / 現在株価 × 100 (目標までの余地) |
| PER / PBR    | **Track A**         | yfinance `trailingPE` / `priceToBook`               |
| 次回決算日   | **Track A**         | next earnings date from `Ticker.calendar`           |
...
Track A writes `現在株価` / `配当利回り` / `配当金` plus the derived `評価額` /
`評価損益額` / `評価損益率（%）` / `目標との乖離率（%）` and `PER` / `PBR` / `次回決算日`
here (any of these whose header is absent in the sheet is silently skipped). Track B =
the **holdings-review** skill writes only `AIコメント`.
...
`Ticker.calendar` ("Earnings Date"), かぶたんURL / みんかぶURL from the ticker string
(no fetch; みんかぶ JP=`minkabu.jp/stock/{code}`, US=`us.minkabu.jp/stocks/{TICKER}`),
時価総額 by normalising `Ticker.info` `marketCap` to **億円** (FX-converted to JPY
for non-JPY listings; unit unified across JP and US tabs), PEG / 52週高安 verbatim
from `Ticker.info` (`trailingPegRatio` / `fiftyTwoWeekHigh` / `fiftyTwoWeekLow`), and
the computed 乖離率/位置 columns: 購入検討との乖離率（%）= (現在株価 − 購入検討株価)/購入検討株価×100,
52週レンジ内位置（%）= (現在 − 安値)/(高値 − 安値)×100, アナリスト予想乖離率（%）=
(アナリスト予想株価 − 現在株価)/現在株価×100 (アナリスト予想株価 is a Track B value, so
this is `N/A` until Track B fills it and Track A reruns), and the rest verbatim
...
- Numeric columns carry Google Sheets **number formats** (cells keep raw, sortable
  numbers; only the display is styled): ratios (PER/PBR/業界PER/業界PBR/PEGレシオ) →
  `0.00`; percent magnitudes (配当利回り/年間EPS前年比/評価損益率（%）/各乖離率/52週レンジ
  内位置（%）) → `0.00"%"` (value is already the percent magnitude, e.g. `2.34` = 2.34%,
  so do **not** use a PERCENT format); EPS → `#,##0.00`; prices and price-derived
  amounts (52週高値/52週安値/評価額/評価損益額) → `#,##0` on JP tabs / `#,##0.00` on US
  tabs and the holdings tab; 時価総額 → `#,##0"億円"`. Write raw numbers (and percent values as their
...
- The comment is an **opinionated verdict**, not a restatement of the figures —
  ...
  短中長期 / 目標売却株価; stock-research weighs the watch candidate against the
